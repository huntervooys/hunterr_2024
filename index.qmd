...
Quarto also lets me show you "paged tables" which I think are kind of a neat idea. Unfortunately, I don't think they're all that helpful. Call this lesson one. In the following code, I generate 1000 random numbers between -10 and 10, which I label `x`. Then, I apply the `mystery_function` to each number, producing `y`. What does the mystery function do?
...
All of the chunks of code have line numbers (because I intend for you to ask me specific questions about them). For any chunk, a little clipboard appears when you hover over the code listing, and you can copy it by clicking on the clipboard. (This includes the code in the "View Source" pane, meaning you can copy the entire page, text and all.)

Quarto also processes citations, so I can, for example, easily direct you towards my two fathers: @turingComputableNumbersApplication1936 and @foucaultHistorySexuality1978. It will also process links so that I can, for example, send you back to the top of the page: @lst-first.
...
-   RStudio: a graphical (user) interface (GUI) and interactive development environment (IDE) with which to use R

-   tidyverse: a suite of data analysis software packages (collections of functions) within R that makes R code more consistent and easier to read

-   the grammar of graphics: a suite of software packages (that overlap with the tidyverse) that are useful for preparing graphics (like plots, maps, charts, etc.)
...
We are also going to do statistics! There are two approaches to statistics that we're going to adopt through these lessons, so I'd like to begin by elucidating the way in which these approaches are different. The first approach is for pretentious people - scientists and statistical purists. I intend to show you some of their formal techniques, like a variety of hypothesis tests. **The ultimate goal of both approaches is to predict the result of measurements.**

-   machine learning: If the purpose of a statistical model is to predict data, then a model that makes the most accurate prediction is the best model. The model creation process is iterative. Once you see the results of a model, you can use the results to modify the model itself. Generally, practitioners recognize that there are a variety of different types of models that could be used for any task. Thus, they usually construct a variety of different models and then compare them to select a final model. Practitioners will use numbers (in diagnostic and statistic tables) or visuals (like a residuals plot) when comparing models.

-   "real" statistics: In the best case, at least according to the "Open science" framework, the final statistical model will have been selected and preregistered before data is even collected. Statistics have to be rigorous to mean anything, a fact which the machine learning people ignore. They don't check model assumptions using statistical tests like the Shapiro-Wilk normality test, and they are constantly "p-hacking" and "HARKing" to forcibly extract findings from their data. They probably can't even do the math!

I'm often somewhat flippant about the "real" statistics approach, which I think the "real" statisticians absolutely deserve as a result of their relentless gate-keeping. It is clear that their approach, relative to the machine learning approach, is on the way out. Machine learning is progressing in leaps and bounds (at least right now) while the "real" statisticians argue with each other about causality or whatever.

We won't throw the baby out with the bathwater, however. I have no intention of showing you how to do pure, "real" statistics because I do not pretend to really know or care how it is performed; but, many of the tools of "real" statistics are helpful or thought-provoking, and I'll try to bring them in within a framework that is more aligned with a computational or machine learning approach (e.g., using Monte Carlo simulations to do sample size calculations for traditional statistical tests).

You should not think of these two approaches as completely separate and non-overlapping. They overlap quite a bit, in fact. The difference I want to draw out is context. Although machine learning people have become fully embedded within academia, I think the vast majority of "real" statisticians are academics. A professor is more likely to ask you to do "real" statistics than an employer, who is likely interested in elucidating a working hypothesis rather than a scientific "truth." My intention is to give you a basic foundation of knowledge and ability in R that you will allow you to efficiently evaluate, learn about, and complete either of these tasks.
